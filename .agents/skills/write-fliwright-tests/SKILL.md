...
把 Fliwright 测试当作**确定性的 Flutter app 自动化**来写，而不是浏览器测试。普通验证用 `@fliwright/vitest` 的 `test` fixture；一次性自动化、数据录入、录制脚本清理优先用新版 `script` fixture。新脚本默认使用 timeline-native `{ flow, mock, agent, logger }` 和 Playwright-style `expect(locator, title?)`，让运行产出 `timeline.json`、`logs/events.jsonl`、截图/快照和 agent-visible failure。
